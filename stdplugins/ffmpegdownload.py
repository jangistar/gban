...
@borg.on(admin_cmd(pattern="dffmpeg ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
    input_str = event.pattern_match.group(1)
    if not os.path.isdir(Config.FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        os.makedirs(Config.FF_MPEG_DOWN_LOAD_MEDIA_PATH)
    if event.reply_to_msg_id:
        ...
        to_download_directory = Config.FF_MPEG_DOWN_LOAD_MEDIA_PATH
        if "|" in input_str:
