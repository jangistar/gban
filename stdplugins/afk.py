...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"My Master **👑 [{}](tg://user?id={user_id}) `{}`** Is **AFK since** {afk_since} " + \
            f"\n\n__and HE may be back soon__\n\n**Because my King is** {reason}" \
            if reason \
            else f"My King **👑 [{}](tg://user?id={user_id}) `{}`** is **AFK Since** {afk_since} so wait until He is back.\n\n**THANKS**."
