...
@borg.on(admin_cmd(pattern="web ?(.+?|) (?:--)(anonfiles|transfer|filebin|anonymousfiles|megaupload|bayfiles)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if input_str:
        ...
        file_name = await bot.download_media(reply.media, Config.TMP_DOWNLOAD_DIRECTORY)
