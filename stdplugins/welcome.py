...
async def welcome_to_chat(event):
    try:
        ...
        from sql_helpers.welcome_sql import update_previous_welcome
...
@borg.on(admin_cmd(pattern="checkwelcome(.*)"))
...
@borg.on(admin_cmd(pattern="rmwelcome(.*)"))
async def del_welcome(event):
    try:
        from sql_helpers.welcome_sql import rm_welcome_setting
    except AttributeError:
        await event.edit("`Running on Non-SQL mode!`")
        return
